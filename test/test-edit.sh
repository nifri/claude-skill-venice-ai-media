...
echo "=== venice-image.sh edit tests ==="
echo ""
...
output="$(env -u VENICE_API_KEY HOME=/nonexistent bash "$SCRIPT" edit -p "test" -i "$TEMP_INPUT" -o /tmp/out.png 2>&1)" && fail "should exit non-zero" || {
...
output="$(VENICE_API_KEY=fake bash "$SCRIPT" edit -i /tmp/fake.png -o /tmp/out.png 2>&1)" && fail "should exit non-zero" || {
...
output="$(VENICE_API_KEY=fake bash "$SCRIPT" edit -p "test" -o /tmp/out.png 2>&1)" && fail "should exit non-zero" || {
...
output="$(VENICE_API_KEY=fake bash "$SCRIPT" edit -p "test" -i /tmp/fake.png 2>&1)" && fail "should exit non-zero" || {
...
output="$(VENICE_API_KEY=fake bash "$SCRIPT" edit -p "test" -i /tmp/nonexistent-image-12345.png -o /tmp/out.png 2>&1)" && fail "should exit non-zero" || {
...
output="$(bash "$SCRIPT" edit --help 2>&1)" || true
