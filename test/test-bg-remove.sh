...
echo "=== venice-image.sh bg-remove tests ==="
echo ""
...
output="$(env -u VENICE_API_KEY HOME=/nonexistent bash "$SCRIPT" bg-remove -i "$TEMP_INPUT" -o /tmp/out.png 2>&1)" && fail "should exit non-zero" || {
...
output="$(VENICE_API_KEY=fake bash "$SCRIPT" bg-remove -o /tmp/out.png 2>&1)" && fail "should exit non-zero" || {
...
output="$(VENICE_API_KEY=fake bash "$SCRIPT" bg-remove -i /tmp/fake.png 2>&1)" && fail "should exit non-zero" || {
...
output="$(VENICE_API_KEY=fake bash "$SCRIPT" bg-remove -i /tmp/nonexistent-image-12345.png -o /tmp/out.png 2>&1)" && fail "should exit non-zero" || {
...
output="$(bash "$SCRIPT" bg-remove --help 2>&1)" || true
