...
echo "=== venice-image.sh styles tests ==="
echo ""

# Test 1: Missing API key
echo "Test: Missing API key produces error"
unset VENICE_API_KEY 2>/dev/null || true
output="$(env -u VENICE_API_KEY HOME=/nonexistent bash "$SCRIPT" styles 2>&1)" && fail "should exit non-zero" || {
...
output="$(VENICE_API_KEY=fake bash "$SCRIPT" styles --bogus 2>&1)" && fail "should exit non-zero" || {
...
output="$(bash "$SCRIPT" styles --help 2>&1)" || true
